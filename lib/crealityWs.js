...
const { setTimeout: delay, clearTimeout: cancelDelay } = require('node:timers');
...
class CrealityWsClient extends EventEmitter {
	...
	constructor(opts) {
		...
		this.port = opts.port;
		this.log = opts.log;
		this._delay = typeof opts.setTimeout === 'function' ? opts.setTimeout : delay;
		this._cancelDelay = typeof opts.clearTimeout === 'function' ? opts.clearTimeout : cancelDelay;
		this.socket = null;
	...
	stop() {
		...
		if (this.reconnectTimer) {
			this._cancelDelay(this.reconnectTimer);
			this.reconnectTimer = null;
		}
		if (this.socket) {
			try {
	...
	connect() {
		...
		try {
			...
			this.log.error(/** @type {Error} */ (e).message);
			return;
		}
		...
		try {
			...
			if (isUnreachableError(e)) {
				if (this.reachable) {
					this.log.info(`Creality WS unreachable: ${/** @type {Error} */ (e).message}`);
					this.reachable = false;
				}
			} else {
				this.log.warn(`Creality WS open: ${/** @type {Error} */ (e).message}`);
			}
	...
	scheduleReconnect() {
		...
		this.reconnectTimer = this._delay(() => {
			this.reconnectTimer = null;
			this.connect();
		}, 5000);
	}
